...
*   テスター：基本的な電気量の測定、コンデンサの容量や周波数など
*   オシロスコープ：信号の波形を視覚化する
*   ロジックアナライザ

基本的には自宅IoTをやる計画しかなく、第4章以降はあまりピンとこなかった。いや、多分セキュリティ周りは必須なんだろうが…もう少し手を動かしたあとにまた読み直そう。

最後のPoCのところはIoT以外にも通用する一般的な内容でありながら、よりIoTならではの意識するべきポイントとしてまとめられていてなるほどなぁと思った。デバイス発注するならある程度のロットが必要だけど、発注したあとに不整合が発覚したらマズイのでまずはありもののデバイスでPoCしつつ、実現可能性を確かめるみたいなのとか。

やっぱり、IoTを最大限やるならデバイスを作る側に回りたい。デバイスとソフトウェア両方やりたいなぁ。
