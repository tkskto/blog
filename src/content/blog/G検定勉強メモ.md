...
最後まで実行し、得られた総報酬の平均で価値や方策を評価する方法。

分散は大きいが実装は単純。REINFORCEはMC方策勾配の代表で、MCTSはモンテカルロを用いた計画手法にあたる。

#### TD学習

実サンプルから1ステップ先の推定値で更新する（ブートストラップする）。モデル不要で逐次学習でき、サンプル効率が良い。
...
重みの初期設定法や、量子化、プルーニング、蒸留などによるモデルの軽量化などの他にも、
...
画像の各ピクセルにクラスを割り当てる。道路・人・建物などを色分けするが、同じクラスの個体は区別しない。

#### 完全畳み込みネットワーク（FCN）

全結合層を畳み込みに置き換え、入力と同サイズの予測マップを出す。ダウンサンプリング後にアップサンプリングとスキップ接続で細部を復元する。

### インスタンスセグメンテーション

物体ごとにピクセル単位のマスクを出す。セマンティックセグメンテーションと違い、同じクラスでも個体を分離する。
